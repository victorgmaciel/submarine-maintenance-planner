...
import { Users, Wrench, AlertTriangle, Calendar } from "lucide-react";
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded-lg transition font-semibold"
            disabled={currentDay === 1}
          >
            ← Previous Day
          </button>
          <div className="bg-slate-900 border-2 border-yellow-500 rounded-lg px-5 py-3">
            ...
            <input
              ...
              onChange={(e) =>
                setCurrentDay(
                  Math.max(1, Math.min(viewDays, parseInt(e.target.value) || 1))
                )
              }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/60 border-2 border-blue-500/50 rounded-lg p-4 hover:border-blue-500 transition">
            <div className="text-blue-400 text-sm font-bold mb-2 uppercase tracking-wider">
              Officer of the Day
            </div>
            <div className="text-white text-xl font-bold">
              {dailyInfo[currentDay]?.ood || "Not Assigned"}
            </div>
          ...
          <div className="bg-slate-900/60 border-2 border-green-500/50 rounded-lg p-4 hover:border-green-500 transition">
            <div className="text-green-400 text-sm font-bold mb-2 uppercase tracking-wider">
              Chief of the Boat
            </div>
            <div className="text-white text-xl font-bold">
              {dailyInfo[currentDay]?.cob || "Not Assigned"}
            </div>
          ...
          <div className="bg-slate-900/60 border-2 border-yellow-500/50 rounded-lg p-4 hover:border-yellow-500 transition">
            <div className="text-yellow-400 text-sm font-bold mb-2 uppercase tracking-wider">
              Duty Section
            </div>
            <div className="text-white text-5xl font-bold text-center py-2">
              {dailyInfo[currentDay]?.section || "N/A"}
            </div>
        ...
        {/* Right Column - Operations of the Day */}
            ...
            <div className="text-sm text-gray-400">
              {
                tasks.filter(
                  (t) =>
                    currentDay >= t.startDay &&
                    currentDay < t.startDay + t.duration
                ).length
              }{" "}
              Active
            </div>
            ...
            {tasks.filter(
              ...
              tasks
                ...
                .map((task) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className={`${divColors.bg} border-2 ${divColors.border} rounded-lg p-4 hover:shadow-lg transition`}
                    >
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span
                              className={`${divColors.text} text-xs font-bold px-2 py-1 rounded border ${divColors.border}`}
                            >
                              {task.division}
                            </span>
                            <span className="text-xs text-gray-400 bg-slate-900/50 px-2 py-1 rounded">
                              {task.system}
                            </span>
                            <span className="text-xs text-gray-400 bg-slate-900/50 px-2 py-1 rounded">
                              {task.createdBy}
                      ...
                      <div className="mb-2">
                        <div className="flex justify-between text-xs text-gray-400 mb-1">
                          <span>
                            Day {dayProgress} of {task.duration}
                          </span>
                      ...
                      <div className="flex items-center gap-2 flex-wrap mt-3 pt-3 border-t border-slate-700">
                        ...
                        {Object.entries(task.requirements).map(
                          ([resource, state]) => {
                            if (resource === "crew") {
                              return (
                                <span
                            ...
                            if (state !== "none") {
                              ...
                              return (
                                <span
                                  key={resource}
                                  className={`text-xs px-2 py-1 rounded border ${stateColor} capitalize`}
                                >
                                  {resource}: {state}
                                </span>
                              );
              ...
              <div className="text-center py-16 text-gray-500">
                ...
                <p className="text-sm mt-1">
                  All systems nominal for Day {currentDay}
                </p>
              </div>
            )}
          </div>
